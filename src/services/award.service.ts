...
export default class AwardService extends BaseRepository<DbType> {
  ...
  public async getAllAwards(): Promise<IAward[]> {
    try {
      const allAwards = await this.getAll<IAward, awards>(awardCollection.transformCollection);
      return allAwards;
    } catch (error) {
      throw error;
    }
  ...
  public async getAward(awardId: string): Promise<IAward> {
    try {
      const award = await this.get<IAward, awards>(awardId, awardResource.transform);
      return award;
    } catch (error) {
      throw error;
    }
  ...
  public async updateAward(awardId: string, payload: Partial<IAward>): Promise<IAward> {
    try {
      ...
      const updatedAward = await this.update<IAward, awards>(
        awardId,
        {
          ...(name ? { name } : {}),
